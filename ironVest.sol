...
contract VestingHarvestContarct is AccessControl, ReentrancyGuard {
    ...
    mapping(uint256 => mapping(address => Vesting.UserCliffInfo))
        public UserCliffInfo;
    mapping(uint256 => mapping(address => Vesting.UserNonCliffInfo))
    ...
    function addVesting(
        ...
        for (uint256 i = 0; i < _usersAddresses.length; i++) {
            ...
            uint256 releaseRate = SafeMath.div(
                _userAlloc[i],
                (SafeMath.sub(_vestingTime, block.timestamp))
            );
            poolInfo[vestingPoolSize] = Vesting.PoolInfo(
                _poolName,
                block.timestamp,
                _vestingTime,
                _tokenAddress,
            ...
            userInfo[vestingPoolSize][_usersAddresses[i]] = Vesting.UserInfo(
                _userAlloc[i],
                0,
                _userAlloc[i],
                block.timestamp,
    ...
    function poolInfoArrays(uint256 poolIndex)
        internal
        view
        ...
        )
    {
        ...
        if (poolInfo[_poolId].startDate < block.timestamp) {
            if (poolInfo[_poolId].vestingTime < block.timestamp) {
                ...
                claimable = SafeMath.mul(
                    SafeMath.sub(block.timestamp, info.lastwithdrawal),
                    releaseRate
                );
            }
        } else {
    ...
    function claim(uint256 _poolId) public nonReentrant {
        (
            ...
            uint256 secDiff,
            uint256 releaseRate
        ) = claimable(_poolId, _msgSender());
        IERC20(poolInfo[_poolId].tokenAddress).transfer(
            _msgSender(),
            transferAble
        );
        ...
        require(
            ...
            "Vesting: Lock Time Is Not Over Yet"
        );
        require(transferAble > 0, "Vesting: Invalid TransferAble");
        uint256 claimed = SafeMath.add(info.claimedAmount, transferAble);
        userInfo[_poolId][_msgSender()] = Vesting.UserInfo(
            info.allocation,
            claimed,
            SafeMath.sub(info.allocation, claimed),
            block.timestamp,
    ...
    function addCliffVesting(
        ...
        require(
            _vestingTime > block.timestamp,
            "Vesting: Vesting Time Must Be Greater Than Current Time"
        );
        require(
            _vestingTime > _cliffPeriod,
            "Vesting: Vesting Time Time Must Be Greater Than Cliff Period"
        );
        require(
            _cliffVestingTime < _vestingTime,
            "Vesting: Cliff Vesting Time Must Be Lesser Than Vesting Time"
        );
        require(
            _cliffVestingTime > _cliffPeriod,
            "Vesting: Cliff Vesting Time Must Be Greater Than Cliff Period"
        );
        require(
            signatureVerification(signature, _salt) == signer,
            "Signer: Invalid signer"
        ...
        uint256 nonCliffVestingTime = SafeMath.add(
            SafeMath.sub(_vestingTime, _cliffVestingTime),
            _cliffPeriod
        );
        uint256 totalVesting;
        for (uint256 i = 0; i < _usersAddresses.length; i++) {
            ...
            cliffPoolInfo[vestingPoolSize] = Vesting.CliffPoolInfo(
                _poolName,
                block.timestamp,
                _vestingTime,
                _cliffVestingTime,
            ...
            UserCliffInfo[vestingPoolSize][_usersAddresses[i]] = Vesting
                .UserCliffInfo(
                    _userAlloc[i],
                    SafeMath.div(
                        (SafeMath.mul(_userAlloc[i], _cliffPercentage)),
                        100
                    ),
                    0,
                    _cliffPeriod,
                    ...
                    SafeMath.div(
                        ...
                        SafeMath.sub(_cliffVestingTime, _cliffPeriod)
                    ),
                    _cliffPeriod
                );
            userNonCliffInfo[vestingPoolSize][_usersAddresses[i]] = Vesting
                .UserNonCliffInfo(
                    _userAlloc[i],
                    SafeMath.sub(
                        _userAlloc[i],
                    ...
                    SafeMath.div(
                        ...
                        SafeMath.sub(nonCliffVestingTime, _cliffPeriod)
                    ),
                    _cliffPeriod
                );
        }
        ...
        )
    {
        ...
        uint256 secondDiff;
        uint256 releaseRate;
        Vesting.UserCliffInfo memory info = UserCliffInfo[_poolId][_user];
        require(
            info.allocation > 0,
            "Allocation: You Don't have allocation in this pool"
        );
        ...
        if (cliffPoolInfo[_poolId].cliffPeriod < block.timestamp) {
            if (cliffPoolInfo[_poolId].cliffVestingTime < block.timestamp) {
                ...
                cliffPoolInfo[_poolId].cliffVestingTime > block.timestamp
            ) {
                cliffClaimable = SafeMath.mul(
                    SafeMath.sub(block.timestamp, info.clifflastwithdrawal),
                    info.cliffReleaseRatePerSec
                );
                secondDiff = SafeMath.sub(
                    block.timestamp,
                    info.clifflastwithdrawal
                );
                releaseRate = info.cliffReleaseRatePerSec;
            }
        ...
        )
    {
        ...
        uint256 secondDiff;
        uint256 releaseRate;
        Vesting.UserNonCliffInfo memory info = userNonCliffInfo[_poolId][_user];
        require(
            info.allocation > 0,
            "Allocation: You Don't have allocation in this pool"
        );
        ...
        if (cliffPoolInfo[_poolId].cliffPeriod < block.timestamp) {
            if (cliffPoolInfo[_poolId].nonCliffVestingTime < block.timestamp) {
                ...
                cliffPoolInfo[_poolId].nonCliffVestingTime > block.timestamp
            ) {
                nonCliffClaimable = SafeMath.mul(
                    SafeMath.sub(block.timestamp, info.nonClifflastwithdrawal),
                    info.noncliffReleaseRatePerSec
                );
                secondDiff = SafeMath.sub(
                    block.timestamp,
                    info.nonClifflastwithdrawal
                );
    ...
    function claimCliff(uint256 _poolId) public nonReentrant {
        Vesting.UserCliffInfo memory info = UserCliffInfo[_poolId][
            _msgSender()
        ];
        require(
        ...
        (
            ...
            uint256 secondDiff,
            uint256 releaseRate
        ) = cliffClaimable(_poolId, _msgSender());
        require(transferAble > 0, "Vesting: Invalid TransferAble");
        IERC20(cliffPoolInfo[_poolId].tokenAddress).transfer(
            _msgSender(),
            transferAble
        );
        uint256 claimed = SafeMath.add(transferAble, info.claimedAmnt);
        UserCliffInfo[_poolId][_msgSender()] = Vesting.UserCliffInfo(
            info.allocation,
            info.cliffAlloc,
            claimed,
            info.tokensReleaseTime,
            SafeMath.sub(info.cliffAlloc, claimed),
            info.cliffReleaseRatePerSec,
            block.timestamp
        );
        emit CliffClaim(
    ...
    function claimNonCliff(uint256 _poolId) public nonReentrant {
        Vesting.UserNonCliffInfo memory info = userNonCliffInfo[_poolId][
            _msgSender()
        ];
        require(
            cliffPoolInfo[_poolId].cliffPeriod < block.timestamp,
        ...
        (
            ...
            uint256 secondDiff,
            uint256 releaseRate
        ) = nonCliffClaimable(_poolId, _msgSender());
        uint256 claimed = SafeMath.add(transferAble, info.claimedAmnt);
        require(transferAble > 0, "Vesting: Invalid TransferAble");
        IERC20(cliffPoolInfo[_poolId].tokenAddress).transfer(
            _msgSender(),
        ...
        userNonCliffInfo[_poolId][_msgSender()] = Vesting.UserNonCliffInfo(
            info.allocation,
            info.nonCliffAlloc,
            claimed,
            info.tokensReleaseTime,
            SafeMath.sub(info.nonCliffAlloc, claimed),
            info.noncliffReleaseRatePerSec,
            block.timestamp
        );
        emit NonCliffClaim(
